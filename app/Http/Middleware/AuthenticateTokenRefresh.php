...
/**
 * Allows token refresh using a valid (possibly recently expired) JWT so active
 * users can keep a sliding session without being forced to sign in again.
 */
class AuthenticateTokenRefresh
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            $payload = $this->jwtTokenService->decodeForRefresh($token);
        ...
        $request->attributes->set('jwt_remember', ! empty($payload['remember']));
        Auth::setUser($admin);
