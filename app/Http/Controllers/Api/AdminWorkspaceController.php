...
class AdminWorkspaceController extends Controller
{
    ...
    public function create(Request $request, string $business_client_id): JsonResponse
    {
        ...
        try {
            ...
            if ($e->getStatus() === 400 && str_contains(strtolower($upstreamDetail), 'workspace already exists')) {
                ...
                return response()->json($this->toWorkspaceOut($workspace, $business), 201);
            }

            return response()->json([
                'detail' => 'Unable to create workspace. Please try again.',
    ...
    public function index(Request $request, string $business_client_id): JsonResponse
    {
        ...
        return response()->json(
            $workspaces->map(fn (Workspace $workspace): array => $this->toWorkspaceOut($workspace, $business))->values()
        );
    }
    ...
    public function show(Request $request, string $business_client_id, string $workspace_id): JsonResponse
    {
        ...
        return response()->json($this->toWorkspaceOut($workspace, $business));
    }
    ...
    public function update(Request $request, string $business_client_id, string $workspace_id): JsonResponse
    {
        ...
        try {
            $this->projectApiService
                ->withToken($jwtToken)
                ->updateWorkspace($business->business_client_id, $workspace->workspace_id, [
                    'name' => $payload['name'],
                ]);
        } catch (ProjectApiException $e) {
            if (! in_array($e->getStatus(), [404, 400, 403], true)) {
                return response()->json([
                    'detail' => 'Unable to update workspace. Please try again.',
                    'code' => 'workspace_sync_failed',
    ...
    public function delete(Request $request, string $business_client_id, string $workspace_id): JsonResponse
    {
        ...
        try {
            ...
            if ($e->getStatus() !== 404) {
                return response()->json([
                    'detail' => 'Unable to delete workspace. Please try again.',
    ...
    private function toWorkspaceOut(Workspace $workspace, ?Business $business = null): array
    {
        return [
            'workspace_id' => $workspace->workspace_id,
            'name' => $workspace->name,
            'business_client_id' => $workspace->business_client_id ?: $business?->business_client_id,
