...
class RagController extends Controller
{
    ...
    public function retrieve(Request $request): JsonResponse
    {
        ...
        if (! \App\Services\StaffAccess::canAccessBusiness($admin, $business) && $admin->role !== 'user') {
            return response()->json(['detail' => 'Not allowed'], 403);
        }
        if ($admin->role === 'user' && $business->admin_id !== null && $admin->business_id !== $business->id) {
            return response()->json(['detail' => 'Not allowed'], 403);
        }
    ...
    {
        if (\App\Services\StaffAccess::canAccessBusiness($admin, $business)) {
            return true;
        }
